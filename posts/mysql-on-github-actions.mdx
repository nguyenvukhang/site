...
summary: 'Using CI with database functions testing'
...
MySQL instance to run server-side GitHub Actions took me far longer than
whatever I've done locally, and so that's why I'm staying up an extra
hour to write this quick guide so the next person doing the exact same
thing doesn't have to go through the 50-odd failed GitHub Action runs
...
<summary>Full working yaml file (this goes into
<code>.github/workflows/cool-name.yml</code>)</summary>
...
Don't mind the emojis - those are customary in GitHub Actions by now.
