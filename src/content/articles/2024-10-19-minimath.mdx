...
description: Adventures writing a textbook, part 2
pubDate: '19 Oct 2024'
tags: [math]
---
...
mathematics. When I started out in middle/high school I could still vaguely
remember which theorems I've proved and how, and that helped me navigate
...
1. <span id="navigatable">**Navigatable**</span>. I like it when a set of notes
   directs you easily to where you need to find information. It mildly annoys me
   when the author says things like "Clearly, &lt;statement&gt;" but it's not
   clear to me why "&lt;statement&gt;" is true and I have to search every
   preceding page for a justification.
...
5. <span id="reproducible">**Reproducible**</span>. I want to open-source this
   set of notes eventually, and so I want them to be as future-proof as
   possible. Deploying on a static site with any modern JavaScript framework is
   a little too risky to me in this regard. Moreover, if someone wants to
   download a local copy of the notes, they'd have to run their own server and
   browse the notes on that.

And so I present, after many failed iterations of the same idea, the math
textbook I proudly call &#10024;minimath&#10024;.

# minimath

After no deliberation at all, I've decided to title this project "minimath"
after no one in particular. Here are its main components:
...
1. The user selects a
   [preset](https://www.notion.so/minimath-11319d3c85f680ada091f0f32620547b?pvs=21)
   to compile. If none is specified, `minimath` compiles everything.
...
It will enter a [REPL](https://wikipedia.org/wiki/read-eval-print_loop)
...
this implementation is that we can only tell it to run `pdflatex` on specific
`*.tex` files. That is, we need to specifically export a `main.tex` that
contains _everything_ in the book, and then process that with `pdflatex`. This
breaks the core [compile flow](#compile-flow), and requires us to implement a
secondary one that will only run on CI. Having an extra compile flow creates a
separation between the PDF that's built while writing locally and the PDF that
is exported on CI runs. So while this solution works (and was _the_ solution)
for a long time, I knew that I had to continue searching for a better one.
...
By creating an executable file called `pdflatex.sh` with the contents
...
we can actually use `pdflatex` _as if it were installed_ on the local machine,
by running `./pdflatex.sh` with all the arguments we would normally pass into
`pdflatex`. The idea here being that we link the current working directory,
captured by `$PWD`, to the `/tmp` directory in the container using the `-v`
flag; the `--env` flag sends environment variables defined in the host machine
into the container; and finally we spawn the `pdflatex` command and pass on all
the arguments with `$@`.

This is the current method of operation on GitHub Actions. Interestingly, based
on the 0-1s pull time of the GHCR image, it seems like GHCR-hosted images are
cached. Either way, cached or not, `pdflatex` is now made available on every CI
run in effectively no time at all. So now we have a way to get LaTeX installed
both quickly _and_ predictably. We rebuild the `texlive-small` image only once a
month to pull latest updates for LaTeX itself, but otherwise every build in that
month uses the exact same copy of LaTeX, it coming from the image we built for
that month.
...
- A **Preset** is _named_ a list of **Chapters**.
...
- <span id="mark" /> A **Mark** is a Theorem/Lemma/Remark/etc.

Presets are specified in `config.yml`. These allow us to choose specific subsets
of the entire book to compile, so we can enjoy faster real-time preview builds.
...
always subject to change. I've hence decided to go with just naming the Chapter
and the Subsection. The role of the Section is thus reduced to just _grouping_
similar Subsections together.
...
Putting this all together in an example, consider the file
`defs/linear_algebra/vector_spaces.tex`. It corresponds to exactly on
...
With this, we can refer to marks by their SHA, which makes the code much cleaner
than if we were to use some serialized form of the theorem's name (e.g.
`taylors-theorem`), or an index number (e.g. `T1024` or even `T1.2.4`):
...
the numbering at PDF build-time.
...
And so these are called SHAs in the project. Now if you see a line of Rust that
goes
...
Running `minimath label` will automatically append `\label{...}` to every
unlabeled [mark](#mark) line.

## GitHub Actions

- Run unit tests and integration tests (written in Rust).
- Install `pdflatex` by downloading the `texlive-small` docker image then
  linking to it. `texlive-small` exists because there are no GitHub Action
  runner images that come with LaTeX pre-installed, and installing even the
  minimum LaTeX configuration takes unrealistically long for CI/CD runs.
- Build the entire book's PDF.
- Check that all files are formatted, using `latexindent`.
- Upload build assets to the `pdf` branch, and if it's a SemVer tag, then upload
  the assets to the release assets.

## Development Server

1. User specifies a preset to build and watch.
2. Any time a `*.tex` file changes, the server re-runs the build.

This requires the external crate `notify`, and also a (locally written)
debouncer.
